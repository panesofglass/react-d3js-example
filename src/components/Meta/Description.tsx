...
import { Filters, Income, Salary } from '../../DataHandling';
...
class Description extends React.Component<Props, {}> {
    ...
    allDataForYear(year: number, data: Salary[] = this.props.allData) {
        return data.filter(d => d.submit_date !== null && d.submit_date.getFullYear() === year);
    }

    allDataForJobTitle(jobTitle: string, data: Salary[] = this.props.allData) {
        return data.filter(d => d.clean_job_title === jobTitle);
    }

    allDataForUSstate(USstate: string, data: Salary[] = this.props.allData) {
    ...
    get previousYearFragment() {
        ...
        if (year === '*') {
            ...
            let lastYear = this.allDataForYear(Number(year) - 1);
            ...
            if (this.props.data.length / lastYear.length > 2) {
                ...
                const percent = parseFloat(((1 - lastYear.length / this.props.data.length) * 100).toFixed());
    ...
    get countyFragment() {
        ...
        let ordered = _.sortBy(
            ...
            items => (d3mean(items,
                             d => d.base_salary) as number) - medians[items[0].countyID][0].medianIncome);
        ...
        ordered = _.sortBy(_.keys(byCity)
                           ...
                           items => (d3mean(items, d => d.base_salary) as number) - countyMedian);

        best = ordered[ordered.length - 1];

        const city = S(best[0].city).titleCase().s + `, ${best[0].USstate}`,
              mean = d3mean(best, d => d.base_salary) as number;
    ...
    render() {
        const format = this.format,
              mean = d3mean(this.props.data, d => d.base_salary) as number,
              deviation = d3deviation(this.props.data, d => d.base_salary) as number;
